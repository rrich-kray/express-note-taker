...
router.post("/notes", (req, res) => {
  ...
  fs.readFile("./db/db.json", (err, data) => {
    ...
    if (err) throw err;
      ...
      fs.writeFile("./db/db.json", JSON.stringify(parsedTaskData), (err) => {
        // data with newly-added task is rewritten into db.json
        err
          ? console.log(err)
          : `${newTask} has been written to the database file`;
        res.send("Added!");
      });
    }
  });
...
router.delete("/notes/:id", (req, res) => {
  fs.readFile("./db/db.json", "utf8", (err, data) => {
    ...
    let filteredTaskIndex = tasks.indexOf(
      tasks.filter((task) => task.id === req.params.id)
    ); //task is filtered by its id, and its index is assigned to the filteredTaskVariable
    ...
    fs.writeFile(
      ...
      (err) => {
        // Data is then rewritten into db.json
        err
          ? console.log("Error writing information to database file")
